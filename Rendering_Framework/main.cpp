...
bool initializeGL() {
    ...
    // compute shader
    auto rcsShader = std::make_unique<Shader>(GL_COMPUTE_SHADER);
    rcsShader->createShaderFromFile("src/shader/computeShader_reset.glsl");
    std::cout << rcsShader->shaderInfoLog() << '\n';

    ShaderProgram* rcsProgram = new ShaderProgram();
    rcsProgram->init();
    rcsProgram->attachShader(rcsShader.get());
    rcsProgram->checkStatus();
    if (rcsProgram->status() != ShaderProgramStatus::READY)
        return false;
    rcsProgram->linkProgram();
    rcsShader->releaseShader();

    auto csShader = std::make_unique<Shader>(GL_COMPUTE_SHADER);
    csShader->createShaderFromFile("src/shader/computeShader.glsl");
    std::cout << csShader->shaderInfoLog() << '\n';
    ShaderProgram* csProgram = new ShaderProgram();
    csProgram->init();
    csProgram->attachShader(csShader.get());
    ...
    if (!defaultRenderer->initialize(FRAME_WIDTH, FRAME_HEIGHT, shaderProgram, rcsProgram, csProgram)) {
        return false;
    }
...
void paintGL() {
    ...
    // rendering with compute shader & player view		
    ...
    defaultRenderer->computeRenderPass();
